...
const IssuerHome = () => {
  ...
  useEffect(() => {
    const getTokenIds = async (address) => {
      const res = await Interface.Issuer.getIssuedTokenList(address);
      return res.Data;
    };

    const getTokenURIs = async (ids) => {
      const tokenURIs = ids.map(async (tokenId) => {
        const uri = await Interface.Public.tokenURI(tokenId);
        return { tokenId: parseInt(tokenId), tokenURI: uri };
      });

      return Promise.all(tokenURIs);
    };

    const getTokenIPFSContent = async (tokens) => {
      const ipfsContent = tokens.map(async (token, idx) => {
        const httpRes = await axios.get(GATEWAY + token.tokenURI);
        return { ...httpRes.data, tokenId: token.tokenId };
      });
    ...
    if (certificates && certificates.length > 0) {
      return certificates.map((certificate, idx) => (
        <tr key={idx} className="text-gray-700 dark:text-gray-400">
          ...
          <td className="px-4 py-3 text-sm">{certificate.issueDate}</td>
          <td className="px-4 py-3 text-sm">{certificate.issuedTo.name}</td>
              ...
              <table className="w-full whitespace-no-wrap mb-10 text-left">
                <thead>
                  <tr className="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                    {/* <th className="px-4 py-3">TokenID</th> */}
